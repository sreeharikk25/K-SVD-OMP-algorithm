...
void ksvd_denoise(float *image, int width, int height, int patch_size, int dict_size, int sparsity, int max_iter) {
    ...
    for(int iter = 0; iter < max_iter; iter++) {
        ...
        for(int k = 0; k < dict_size; k++) {
            ...
            int *patch_indices = (int*)malloc(sizeof(int)*usage_count);
            int idx = 0;
            for(int p = 0; p < num_patches; p++) {
                if(codes[p*dict_size + k] != 0.0f) {
                    patch_indices[idx++] = p;
            ...
            for(int i = 0; i < usage_count; i++) {
                ...
                float *approx = (float*)malloc(sizeof(float)*patch_dim);
                memset(approx, 0, sizeof(float)*patch_dim);
    ...
    float *accum_image = (float*)malloc(sizeof(float)*width*height);
    float *weight_image = (float*)malloc(sizeof(float)*width*height);
    memset(accum_image, 0, sizeof(float)*width*height);
    memset(weight_image, 0, sizeof(float)*width*height);

    patch_idx = 0;
    for(int y = 0; y < num_patches_y; y++) {
        for(int x = 0; x < num_patches_x; x++) {
            float *recon_patch = &reconstructed_patches[patch_idx*patch_dim];
            for(int dy = 0; dy < patch_size; dy++) {
                for(int dx = 0; dx < patch_size; dx++) {
                    int idx = (y + dy)*width + (x + dx);
                    accum_image[idx] += recon_patch[dy*patch_size + dx];
                    weight_image[idx] += 1.0f;
                }
            }
            patch_idx++;
        }
...
int main(int argc, char *argv[])
{
    ...
    int width = WIDTH;
    int height = HEIGHT;
    float noise_level = 0.1f; // Adjust noise level as needed
    ...
    // Call the K-SVD denoising function
